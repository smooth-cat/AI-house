module.exports = { login, register, getTemperature, shutdown, random };
...
async function login(pool, user) {//登录-注册处理
...
async function register(pool, user) {
    try {
        var success = await insert(pool, user)
        console.log(success)
