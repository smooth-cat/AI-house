...
2. 配置: 打开解压文件夹中的**conf目录**下的nginx.conf文件
   找到
```
        location / {
            root   html;
...
2. 安装数据库时用户名请填root  密码填mysql  
   **如果要定义自己的账号密码**，就把app.js 和 home.js 中的 mysql.createPool() 函数做对应修改 
...
### 4.安装node，启动 app.js 和 home.js
   ...
   运行： 一个cmd/终端输入node app  另一个输入 node home  
   停止运行: ctrl+c 不行就多按几下  
#### node服务器就启动完毕了 大功告成~ 在浏览器 127.0.0.1 就可以操作它了~
